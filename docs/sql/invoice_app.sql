...
CREATE TABLE IF NOT EXISTS `invoice_app`.`customers` (
...
CREATE TABLE IF NOT EXISTS `invoice_app`.`customer_has_addresses` (
  `customer_id` VARCHAR(200) CHARACTER SET 'utf8' NOT NULL,
  `postal_code` VARCHAR(40) CHARACTER SET 'utf8' NOT NULL,
  `number` INT(11) NOT NULL,
  PRIMARY KEY (`customer_id`, `postal_code`, `number`),
  ...
  CONSTRAINT `dha_address_fk`
    ...
    ON DELETE CASCADE,
  CONSTRAINT `dha_customer_fk`
    FOREIGN KEY (`customer_id`)
    REFERENCES `invoice_app`.`customers` (`id`)
    ON DELETE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `invoice_app`.`invoices` (
  ...
  INDEX `invoices_customers_fk` (`customer_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `invoice_app`.`invoice_items` (
  `invoice_number` VARCHAR(200) CHARACTER SET 'utf8' NOT NULL,
...
CREATE TABLE IF NOT EXISTS `invoice_app`.`settings` (
  ...
  `website` VARCHAR(300) CHARACTER SET 'utf8' NULL DEFAULT NULL,
  `phone` VARCHAR(40) CHARACTER SET 'utf8' NULL DEFAULT NULL,
  `email` VARCHAR(150) CHARACTER SET 'utf8' NOT NULL,
  `password` VARCHAR(80) CHARACTER SET 'utf8' NOT NULL,
  `smtp` VARCHAR(80) CHARACTER SET 'utf8' NOT NULL,
  `port` INT(11) NOT NULL,
  `bank_account` VARCHAR(150) CHARACTER SET 'utf8' NOT NULL,
  `bank` VARCHAR(150) CHARACTER SET 'utf8' NOT NULL,
  ...
  `business_number` VARCHAR(200) CHARACTER SET 'utf8' NOT NULL,
  `vat_number` VARCHAR(200) CHARACTER SET 'utf8' NOT NULL,
  `invoice_prefix` VARCHAR(20) CHARACTER SET 'utf8' NULL DEFAULT NULL,
  `logo` TEXT CHARACTER SET 'utf8' NULL DEFAULT NULL,
