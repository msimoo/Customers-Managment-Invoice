...
namespace CustomersManagementApp.Components.Helpers {
	public class Email
    {
        ...
        public async Task SendNotification(Customer customer)
        {
            var smtpClient = new SmtpClient
            {
                Host = this.Host,
            ...
            using (var message = new MailMessage(this.EmailAddress, customer.Email)
            {
                IsBodyHtml = true,
                Subject = String.Format("{0} - New Invoice", this.Settings.CompanyName),
                Body = string.Format("Dear {0}. {1},<br /><br />A new invoice is awaiting your attention.<br /><br />Kind regards,<br /><br />Invoice Panel", customer.FirstName[0], customer.LastName)
            })
            {
                await smtpClient.SendMailAsync(message);
            }
