...
using CustomersManagementApp.Components.Helpers;
    ...
    public class InvoicesController : Controller
    {
        ...
        public async Task<IActionResult> Index(int? page, int? pageSize)
        {
            ...
            foreach (var invoice in data)
            {
                ...
                customerModel.SetProperties(customer, true, true);
        ...
        /// <summary>
        /// Gets an invoice by customer id.
        /// </summary>
        ...
        [ProducesResponseType(typeof(void), 400)]
        [ProducesResponseType(typeof(void), 500)]
        public async Task<IActionResult> GetByCustomerId(string customer)
        {
            ...
            //Get invoices
            var data = await _repo.GetByCustomerId(customer);
            if (data == null)
            {
                return StatusCode(500, "Invoice with customer id " + customer + " could not be found.");
            }
            ...
            foreach (var invoice in data)
            {
                ...
                var customerModel = new CustomerViewModel();
                customerModel.SetProperties(customers, true, true);
        ...
        public async Task<IActionResult> Create([FromBody]InvoiceViewModel model)
        {
            ...
            Invoice invoice = new Invoice
            {
                ...
                Customer = new Customer
                {
                    ...
                    BankAccount = model.Customer.BankAccount,
            ...
            {
                await _email.SendNotification(new Customer
                {
                    Id = model.Customer.Id,
                    FirstName = model.Customer.FirstName,
                    LastName = model.Customer.LastName,
        ...
        public async Task<IActionResult> Update([FromBody]InvoiceViewModel model)
        {
            ...
            {
                await _email.SendNotification(invoice.Customer);
            }
        ...
        /// <summary>
        /// Deletes all invoices belonging to a certain customer id
        /// </summary>
        /// <param name="customer">Id of customer</param>
        [HttpDelete("deleteByCustomerId")]
        [ProducesResponseType(typeof(void), 200)]
        [ProducesResponseType(typeof(void), 400)]
        [ProducesResponseType(typeof(void), 500)]
        public async Task<IActionResult> DeleteByCustomerId(string customer)
        {
            if (String.IsNullOrEmpty(customer))
            {
                return StatusCode(400, "Invalid parameter(s).");
            ...
            //Remove invoice(s)
            var succeeded = await _repo.DeleteByCustomerId(customer);
            if (!succeeded)
            {
                return StatusCode(500, "A problem occured while removing the record. Please try again!");
            }
