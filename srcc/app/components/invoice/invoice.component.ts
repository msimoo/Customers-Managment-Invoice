...
export class InvoiceComponent implements OnInit {
    ...
    search() {
        ...
        if (this.pagedResult) {
            this.pagedResult.data.forEach(f => {
                if (f.customer.company_name == null) {
                    if (f.customer_id.toLowerCase().includes(this.query.toLowerCase())
                        ...
                        || f.customer.email.toLowerCase().includes(this.query.toLowerCase())
                        || f.customer.first_name.toLowerCase().includes(this.query.toLowerCase())
                        || f.customer.last_name.toLowerCase().includes(this.query.toLowerCase())) {
                    ...
                    if (f.customer_id.toLowerCase().includes(this.query.toLowerCase())
                        ...
                        || moment(f.created_on).format('MMMM DD, YYYY').toLowerCase().includes(this.query.toLowerCase())
                        || f.customer.email.toLowerCase().includes(this.query.toLowerCase())
                        || f.customer.company_name.toLowerCase().includes(this.query.toLowerCase())) {
    ...
    async getPage(page: number) {
        await this.invoiceService.index(page).toPromise().then(
            (response) => {
                if (this.currentUser.role_id === 2) {
                    response.data = response.data.filter(f => f.customer_id === this.customer.id);
                }
